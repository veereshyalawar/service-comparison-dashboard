...
# Account: 072528252688 (topmate-prod)
# Region: ap-south-1
...
if ! aws sts get-caller-identity --region "$AWS_REGION" --profile "$AWS_PROFILE" &>/dev/null; then
    echo "Error: AWS profile '$AWS_PROFILE' not configured or session expired."
    echo ""
    echo "Configure with:"
    echo "  aws configure --profile topmate-prod"
    exit 1
fi
...
create_function_url() {
    ...
    if aws lambda get-function-url-config --function-name $LAMBDA_FUNCTION_NAME --region $AWS_REGION --profile $AWS_PROFILE 2>/dev/null; then
...
verify() {
    ...
    aws lambda get-function --function-name $LAMBDA_FUNCTION_NAME --region $AWS_REGION --profile $AWS_PROFILE \
    ...
    echo ""
    echo "Function URL:"
    aws lambda get-function-url-config --function-name $LAMBDA_FUNCTION_NAME --region $AWS_REGION --profile $AWS_PROFILE \
